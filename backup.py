...
# schedule = schedule_table.find({"and":{"cinema":{"$in":["suncity cinema"]}, "movie":{"$in":"kaabil"}}})

customer_table = db["customers"]
partner_table = db["partners"]
market_table = db["markets"]
product_table = db["products"]
arrCustomers = []
arrPartners = []
arrMarkets = []
arrProducts = []
...
for partner in partner_table.find():
    # print "movie = {0}".format(movie)
    arrPartners.append(partner)

for market in market_table.find():
    # print "movie = {0}".format(movie)
    arrMarkets.append(market)

for product in product_table.find():
    # print "movie = {0}".format(movie)
    arrProducts.append(product)

arrCustomers_count = customer_table.find().count()
print("customers array", arrCustomers);
print("partners array", arrPartners);
print("markets array", arrMarkets);
print("products array", arrProducts);
# print("customers count = {0}".format(arrCustomers_count))
# cinemas_table = db["cinemas"]
# c_rows = []
# for cinema in cinemas_table.find():
#     # print "cinema = {0}".format(cinema)
#     c_rows.append(cinema)
...
"tell me who you belong to", "Fuzzylogix",
"tell me who you work for?", "fuzzylogix",
"who is your daddy?", "Partha Sen",
...
"tell me who you are ", "Hi! I am FLBot made with Love by bugASURS",
"tell me about yourself", "Hi! I am FLBot made with Love by bugASURS",
"who are you", "Hi! I am FLBot made with Love by bugASURS",
"who made you", "Hi! I am FLBot made with Love by bugASURS",
...
chatterbot.train([
    ...
    "bye... for more information contact at http://www.fuzzylogix.com/about-fuzzy-logix/contact-us/",
...
def create_response(bot_response):
    if(bot_response=="info_customer"):
        ...
        # return "talk to the point"
    # if(bot_response=="update_context_and_repeat_last_action"):
        # return update_context_and_repeat_last_action()
    # return len(context["customers"])
    # return "hello buddy i am fl bot"
...
def find_customers(message):
    ...
    for customer in arrCustomers:
        # print "movie -> {0}".format(movie)
        # print("message in find_customers is ", message)
        # print("customer in find_customers is ", customer)
        # print("if condition check in final_customers", customer["name"])
        # print("if condition check in final_customers", message.find(customer["name"]))
...
#     movies = []
#     # print "arrCustomers -> {0}".format(arrCustomers)
#     for movie in arrCustomers:
#         # print "movie -> {0}".format(movie)
#         if(message.find(movie["name"])>-1):
